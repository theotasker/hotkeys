...
Import(SourceDir)
{
	...
	if not Send_Text(Name_First, "TEdit8", "Open patient case")
	{
		return "Couldn't enter first name for patient search"
	}
	
	if not Send_Text(Name_Last, "TEdit7", "Open patient case")
	{
		return "Couldn't enter last name for patient search"
	}
	
	if not Send_Text(Name_Clinic, "Edit1", "Open patient case")
	{
    ...
    Loop
	{
		...
		if WinExist("New patient info",, "model")  
		{
			...
			if not Send_Text(Name_First " " Name_Last, "TEdit2", "New patient info") = "Fail")
			{
				return "Couldn't enter full name for patient creation"
			}

			if not Send_Text(Name_First, "TEdit8", "New patient info")
			{
				return "Couldn't enter first name for patient creation"
			}
			
			if not Send_Text(Name_Last, "TEdit7", "New patient info")
			{
				return "Couldn't enter last name for patient creation"
			}
			
			if not Send_Text(Name_Clinic, "Edit1", "New patient info")
			{
		...
		else if WinExist("New patient model set info")
		{
			...
			if not Send_Text(Number_Script, "TEdit5", "New patient model set info")
			{
	...
	if (Filename_Upper != "None")
	{
		...
		if not Send_Text(Filename_Upper, "Edit1", "Open")
		{
	...
	if (Filename_Lower != "None")
	{
		...
		if not Send_Text(Filename_Lower, "Edit1", "Open")
		{
