...
f3::
{
	...
	{
		FileMove, %A_MyDocuments%\Temp Models\%Filename_Upper%, %InputDir%%Filename_Base%Upr.stl
		if !FileExist(InputDir Filename_Base "Upr.stl")
		{
	...
	{
		FileMove, %A_MyDocuments%\Temp Models\%Filename_Lower%, %InputDir%%Filename_Base%Lwr.stl
		if !FileExist(InputDir Filename_Base "Lwr.stl")
		{
			Msgbox,, Importing Finishing Error, Attempted to move STL, but couldn't verify it's location after move
			return
		}
	...
	{
		Gui, Add, Button, x12 y210 w100 h30 gUpper, Upper
		Gui, Add, Button, x112 y210 w100 h30 gLower, Lower
		Gui, Add, Button, x212 y210 w100 h30 gBoth, Both

		Gui, Add, Text, x30 y120 w300 h14 +Center, Upper and lower STLs detected, please select which to import
		Gui, Show, w439 h253, Arch Selection
		
		WinWaitClose, Arch Selection
		...
		{
			FileMove, %A_MyDocuments%\Temp Models\%Filename_Upper%, %InputDir%%Filename_Base%Upr[2].stl
			FileMove, %A_MyDocuments%\Temp Models\%Filename_Lower%, %InputDir%%Filename_Base%Lwr[2].stl

			if !FileExist(InputDir Filename_Base "Upr[2].stl") or !FileExist(InputDir Filename_Base "Lwr[2].stl")
			{
				Msgbox,, Importing Finishing Error, Attempted to move STL, but couldn't verify it's location after move
				return
			}
...
f12::
{
	...
	if (Filename_Upper != "") and (Filename_Lower = "")
	{
		FileMove, %A_MyDocuments%\Temp Models\%Filename_Upper%, %InputDir%%Filename_Base%Upr[1].stl
		if !FileExist(InputDir Filename_Base "Upr[1].stl")
		{
			Msgbox,, Importing Finishing Error, Attempted to move STL, but couldn't verify it's location after move
			return
		}
	...
	{
		FileMove, %A_MyDocuments%\Temp Models\%Filename_Lower%, %InputDir%%Filename_Base%Lwr[1].stl
		if !FileExist(InputDir Filename_Base "Lwr[1].stl")
		{
			Msgbox,, Importing Finishing Error, Attempted to move STL, but couldn't verify it's location after move
			return
		}
...
Ortho_OpenCase()
{
	...
	ControlFocus, TEdit14, Open patient case    
    Sleep, 400
    Send, %scriptnumber%
    Sleep, 400
