...
f7::
{
	...
	for key, filename in Temp_Models_List
	{
		...
		for key, tag in Tags_Upper ; all possible marking for an upper
		{
			if instr(filename, tag)
			{
				...
				UpperModelBat:= " -UpperModel=""%USERPROFILE%\Documents\Temp Models\" Filename_Upper """"
				Pass_check := 1
			}
		}
		for key, tag in Tags_Lower ; all possible marking for an upper
		{
			if instr(filename, tag)
			{
				...
				LowerModelBat:= " -LowerModel=""%USERPROFILE%\Documents\Temp Models\" Filename_Lower """"
				Pass_check := 1
			}
		}
	}
	...
	clinic_under := StrReplace(clinic, " ", "_")
	firstname := StrReplace(firstname, " ", "_")
	lastname := StrReplace(lastname, " ", "_")
	
	OrthoBat := A_MyDocuments "\Automation\Ortho.bat"

	OrthoBatText := "pushd \\3shape-2021\3Shape Ortho System\ScanItManager`nScanItManager.exe -SingleInstance=True -PatientInfoConversionType=Cmdline -PatientID=" clinic_under "_" firstname "_" lastname " -FirstName=" firstname " -LastName=" lastname " -Clinic=""" clinic """ -ModelSetID=" scriptnumber LowerModelBat UpperModelBat

	if FileExist(OrthoBat)
		FileDelete, %OrthoBat%

	FileAppend, %OrthoBatText%, %OrthoBat%
	
	Run, %A_MyDocuments%\Automation\Ortho.bat.lnk
