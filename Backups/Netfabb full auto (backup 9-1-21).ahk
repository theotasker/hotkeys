...
{	
	...
	Sleep, 200
	Send {Enter}
	WinWaitActive, CalculationThreadForm, , 10  ; this is the progress bar for importing an stl
	...
	Click, 50, 50 ; Deselects the model to remove the bounding box
	
	Sleep, 200
	
	; Search for top and bottom edges, find middle of model
	TopEdgeLeveling := Pixel_Search("y", 960, 75, 25, "MD")   ; Custom function at bottom of script
	If(TopEdgeLeveling = "Error")
	{
		return "Error with pixel search for leveling, top edge"
	}
	
	BottomEdgeLeveling := Pixel_Search("y", 960, TopEdgeLeveling + 5, 20, "BG")
	...
	{
		BottomEdgeLeveling := Pixel_Search("y", 960, BottomEdgeLeveling + 5, 25, "BG")
		If(BottomEdgeLeveling = "Error")
		{
			BottomEdgeLeveling := Pixel_Search("y", 960, TopEdgeLeveling + 5, 20, "BG")
			If(BottomEdgeLeveling = "Error")
	...
	ModelCenterY := Round(BottomEdgeLeveling - ((BottomEdgeLeveling - TopEdgeLeveling)/3))
	
	BGPixel := 0x000000
	PixelGetColor, PixelColor, 960, ModelCenterY
	If(PixelColor = BGPixel)
	{
		return "Pixel for leveling was black, not on the model"
	}
	
	BlockInput MouseMove ; Click middle of model to reselect it
    MouseGetPos, x, y
    Click, 960, %ModelCenterY%
    MouseMove, %x%, %y%, 0
    BlockInput MouseMoveOff
	
	Sleep, 1000
	
    BlockInput MouseMove ; Click middle of model to select leveling surface
    MouseGetPos, x, y
    Click, 960, %ModelCenterY%
    MouseMove, %x%, %y%, 0
    BlockInput MouseMoveOff
	...
	if ErrorLevel
	{
		return "Couldn't get focus on netfabb before spinning model"
	}
	...
	Send ^+r  ; open rotate dialogue
	WinWaitActive, Rotate Parts, , 2
	if ErrorLevel
	{
		return "Couldn't open rotate dialogue"
	}
	
	
	Sleep, 200
	Send {delete}  ; delete the 0
	sleep, 200
	If(Arch = "Upr")
	{
		Send 110
	...
	Sleep, 100
	Send {Enter}  ; apply the spin
	
	; Old spinning method using z key
	;If(Arch = "Upr")
	;{
	;	Send {z 7}
	;}
	...
	If(BottomEdge < 500)
	{
		EngravingYPosition := Round(BottomEdge - ((BottomEdge - TopEdge)/2.5))
	}
	Else
	{
	...
	Sleep, 400
	SetTitleMatchMode, 3
	if winexist("Export ") ; attempt to complete the repair inside the export box
	{
		SetTitleMatchMode, 1
		WinActivate, Export
		...
		if ErrorLevel
		{
			return "Couldn't get focus on export repair box"
		}
		
		Send {enter} ; activate the "optimize" button
		Sleep, 2000
		
		ControlGetText, Triangles, Edit2, Export ; get the number of bad triangles and edges
		ControlGetText, Edges, Edit1, Export 
		
		if (Triangles = 0) and (Edges = 0) ; if zero issues remain after optimize, export
		{
			Send {down}
			Sleep, 200
			Send {enter}
		...
		{
			return "Couldn't repair file during export repair"
		}
...
Log_Update(FileName, ErrorStatus, ErrorText)
{
	...
	LogFile := LogDir A_MM "-" A_DD "-" A_YYYY ".txt" 
